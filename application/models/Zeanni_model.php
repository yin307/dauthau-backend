...
class Zeanni_model extends CI_Model
{
        ...
        $sql = "SELECT * FROM
                (
                    ...
                    (
                        ...
                        " . $where . "
                        order by  a1.\"BIDER_SELECTION_ID\", a1.\"VERSION_NUM\" desc
                    ) a
                    WHERE rownum < ((" . $page . " * 100) + 1 )
                )
                WHERE r__ >= (((" . $page . "-1) * 100) + 1)";
    ...
    public function listBidings()
    {
        ...
        $sql = "SELECT * FROM ( 
                    SELECT a.*, rownum r__ 
                    FROM ( 
                        select  a1.\"BIDING_ID\" as \"a1-zn-BIDING_ID\",  NVL(a1.\"PROCURING_NAME\", a1.\"INVESTOR\") as \"a1-zn-PROCURING_NAME\",  a1.\"PACKAGE_NAME\" as \"a1-zn-PACKAGE_NAME\",  a1.\"PROJECT_NAME\" as \"a1-zn-PROJECT_NAME\",  
    ...
    public function listBidPackages_prequalification()
    {
        ...
        $sql = "SELECT * FROM ( 
            SELECT a.*, rownum r__ 
            FROM ( 
                select a1.\"BID_PACKAGE_ID\" as \"a1-zn-BID_PACKAGE_ID\",  NVL(a2.\"PROCURING_NAME\", a1.\"INVESTOR\") as \"a2-zn-PROCURING_NAME\",  
    ...
    public function listBidings_prequalification()
    {
        $page = empty($_GET['page']) ? 1 : (int)$_GET['page'];
        $BUSSINESS_FIELD = empty($_GET['BUSSINESS_FIELD']) ? 'hang-hoa' : $this->db->escape_str(trim($_GET['BUSSINESS_FIELD']));
        $where = " where  a1.\"RESULT_TYPE\" = '1' and a1.\"FIELD\"='" . $BUSSINESS_FIELD . "' ";
        if (!empty($_GET['keySearch'])) {
            $_GET['keySearch'] = $this->db->escape_str(trim($_GET['keySearch']));
            $where .= " and  (a1.\"PROCURING_NAME\" like '%" . $_GET['keySearch'] . "%'
                                    or a1.\"PACKAGE_NAME\" like '%" . $_GET['keySearch'] . "%'
    ...
    public function getFollowList()
    {
        ...
        $token = $this->db->escape_str(trim($arr['x-csrftoken']));
        // $token = '4e4af0a9a46e8a047ce5414561a23182';
