...
class Grocery_crud_model  extends CI_Model  {
    ...
    function get_field_types_basic_table()
    {
    	...
    	$sql = 'SELECT a1.column_name as "Field",a1.data_type as "Type", a1.data_length,
			 a2."KEY" as "Key", null as "Default", \'\' as "Extra",a1.nullable as "Null"
		FROM user_tab_cols a1
		left join (
			SELECT column_name, \'PRI\' "KEY" FROM all_cons_columns WHERE constraint_name = (
			  SELECT constraint_name FROM all_constraints 
			  WHERE UPPER(table_name) = UPPER(\''.$this->table_name.'\') AND CONSTRAINT_TYPE = \'P\'
			)
		) a2 on  a2.column_name = a1.column_name
		WHERE table_name = \''.$this->table_name.'\'';
    	foreach($this->db->query($sql)->result() as $db_field_type)
    	{
			// print_r($db_field_type);
    		...
    		// 	$length = '';
    		// }
    		$db_field_types[$db_field_type->Field]['db_max_length'] = $db_field_type->DATA_LENGTH;
    		$db_field_types[$db_field_type->Field]['db_type'] = !empty($arr[$db_field_type->Type])?$arr[$db_field_type->Type]:$db_field_type->Type;
    		$db_field_types[$db_field_type->Field]['db_null'] = $db_field_type->Null == 'Y' ? true : false;
			$db_field_types[$db_field_type->Field]['db_extra'] = $db_field_type->Extra;
			if($db_field_type->Key=='PRI'){
				$this->set_primary_key($db_field_type->Field,$this->table_name);
			}
    	}
